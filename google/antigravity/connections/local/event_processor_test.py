...
class EventProcessorHelperTest(absltest.TestCase):
  ...
  def test_normalize_wire_path_file_uri(self):
    self.assertEqual(
        os.path.normpath(event_processor.normalize_wire_path("file:///dev/shm/workspace/foo.py")),
        os.path.normpath("/dev/shm/workspace/foo.py"),
    )
...
class LocalConnectionStepFromDictTest(absltest.TestCase):
  ...
  def test_step_from_dict_normalizes_file_uri_arguments(self):
    ...
    self.assertLen(step.tool_calls, 1)
    self.assertEqual(
        os.path.normpath(step.tool_calls[0].args.get("file_path")), os.path.normpath("/dev/shm/workspace/foo.py")
    )
    self.assertNotIn("canonical_path", step.tool_calls[0].args)
    self.assertEqual(
        os.path.normpath(step.tool_calls[0].canonical_path),
        os.path.normpath("/dev/shm/workspace/foo.py"),
    )
